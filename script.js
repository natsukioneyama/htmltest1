...
document.addEventListener("DOMContentLoaded", function () {
  ...
  galleryItems.forEach(function (item) {
    var isMobile = window.matchMedia("(max-width: 768px)").matches; // スマホ判定
  ...
  function openSlideshow(item) {
    ...
    displaySlide(index);
    document.getElementById("slideshow-overlay").style.display = "block";
  }
  ...
  document.getElementById("close-button").addEventListener("click", function () {
    ...
    galleryItems.forEach(function (el) {
      el.classList.remove("active"); // 閉じたらキャプションもリセット
    });
  });
});
