...
document.querySelectorAll(".project a img, .project a video").forEach((el) => {
  el.addEventListener("click", (e) => {
    ...
    // クリックされた画像（or動画）の src を取得
    const clickedSrc = el.getAttribute("src") || el.getAttribute("poster");

    // 配列内でクリックしたメディアのインデックスを探す
    const startIndex = projectGalleries.findIndex(item => item.src === clickedSrc);

    // 全配列をそのまま渡す
    openLightbox(projectGalleries, startIndex >= 0 ? startIndex : 0);
  });   
});
